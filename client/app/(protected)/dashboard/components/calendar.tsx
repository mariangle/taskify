  ...
  function CalendarDateItem(props: DayContentProps & CustomProps) {
    ...
    return (
      <span className={cn(
        ...
        hasSameDate ? 'font-bold' : 'font-normal'
      )}>
      {props.date.getDate()}
        <span
          className={`block w-1 h-1 bg-danger rounded-full mx-auto absolute left-1/2 transform-translate-x-1/2 ${hasSameDate ? 'visible' : 'invisible'}`}
        />
      </span>
    );
  }
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        day: cn(
          "rounded-xl hover:bg-danger",
          "h-9 w-9 p-0 font-normal aria-selected:opacity-100"
        ),
        day_range_end: "day-range-end",
        day_selected:
          "bg-danger text-primary-foreground hover:bg-danger hover:text-primary-foreground focus:bg-danger focus:text-primary-foreground",
